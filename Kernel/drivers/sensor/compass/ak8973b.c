...
/*Reporting when HW_REV >=6, because for that condition only proximity sensor driver will get registered*/
		if (apollo_get_remapped_hw_rev_pin() >= 6) // since REV0.9b
		{
...
void gp2a_report_proximity_value(unsigned int prox_value)
{
	ak8973b_data_t *data = i2c_get_clientdata(this_client);
	input_report_abs(data->input_dev, ABS_DISTANCE, prox_value);
	input_sync(data->input_dev);
	gprintk("[GP2A] Proximity value when interrupt comes = %u\n", prox_value);

}
EXPORT_SYMBOL(gp2a_report_proximity_value);
		...
		unsigned long arg)
{
	...
	//  char *pbuffer = 0;
	//gprintk("start\n");
	//printk("cmd=%x getdata=%x\n", cmd, ECS_IOCTL_GETDATA);
	switch (cmd) {
		case ECS_IOCTL_READ:
...
/* 
 * VNVS: 20-APR'10 
 * atomic_set(&open_flag,0) only when proximity sensor is not enabled.
 * When proximity sensor is enabled, AK Daemon should run, it shouldnt stop,
 * so not setting open_flag = '0'.
 */
	   atomic_set(&open_flag, 0);
	   wake_up(&open_wq);
	}
...
static void ak8973b_init_hw(void)
{
	...
	if(gpio_is_valid(GPIO_MSENSE_nRST)){
		...
		gpio_direction_output(GPIO_MSENSE_nRST, GPIO_LEVEL_HIGH);
	}
//	s3c_gpio_setpull(GPIO_MSENSE_nRST, S3C_GPIO_PULL_NONE);
/*VNVS: 11-MAY'10 Pulling RST pin HIGH instead of keeping in floating state(Pulling NONE)*/
	s3c_gpio_setpull(GPIO_MSENSE_nRST, S3C_GPIO_PULL_UP);
...
static ssize_t ak_fs_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	gprintk("ak_fs_read: success full\n");
			...
			const struct i2c_device_id *id)
{
	...
	/*proximity sensor*/
	input_set_abs_params(akm->input_dev, ABS_DISTANCE, -2, 2, 0, 0);	
