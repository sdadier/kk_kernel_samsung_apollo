...
static int VibeOSKernelTimerProc(void* data)
{
	...
	while(!kthread_should_stop()) {
		VibeDebug("%s: while(!kthread_should_stop()) \n", __func__);
		if(g_bTimerThreadStarted) {
			VibeDebug("%s: if(g_bTimerThreadStarted) \n", __func__);
		...
		if(g_bTimerStarted) {
			VibeDebug("%s: if(g_bTimerStarted) \n", __func__);
...
static void VibeOSKernelLinuxStartTimer(void)
{
	...
	if(!g_bTimerStarted) {
		if(!VibeSemIsLocked(&g_hMutex))
			ret = down_interruptible(&g_hMutex); /* start locked */
