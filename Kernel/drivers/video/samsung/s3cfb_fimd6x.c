...
int s3cfb_set_clock(struct s3cfb_global *ctrl)
{
	...
	src_clk = clk_get_rate(ctrl->clock);//ctrl->clock->parent->rate;
	...
	//sm.kim 20100406 fix clkval setting bug
...
int s3cfb_set_fifo_interrupt(struct s3cfb_global *ctrl, int enable)
{
	...
	if (enable) {
		...
		cfg |=  (S3C_VIDINTCON0_INTFIFO_ENABLE | S3C_VIDINTCON0_INT_ENABLE);
	} else {
...
int s3cfb_set_window_control(struct s3cfb_global *ctrl, int id)
{
	...
	if (win->path != DATA_PATH_DMA) {
		...
		if (id == 1) {
			cfg &= ~(S3C_WINCON1_LOCALSEL_MASK | S3C_WINCON1_VP_ENABLE);

			if (win->local_channel == 0) {
				cfg |= S3C_WINCON1_LOCALSEL_FIMC1;
			} else {
				cfg |= (S3C_WINCON1_LOCALSEL_VP | S3C_WINCON1_VP_ENABLE);
			}
		}
	} else {
		...
		*/
		...
		{
			case 16 : 
				if(pdata->swap & FB_SWAP_HWORD)
					cfg |= S3C_WINCON_HAWSWP_ENABLE;
				break;

			case 24 : 
			case 32 : 
				if(pdata->swap & FB_SWAP_WORD)
					cfg |= S3C_WINCON_WSWP_ENABLE;
				break;
...
int s3cfb_set_buffer_address(struct s3cfb_global *ctrl, int id)
{
	...
	if (fix->smem_start) {
		...
		end_addr   = start_addr      + (var->xres_virtual * (var->bits_per_pixel / 8) * var->yres);
		*/
		switch(var->bits_per_pixel)
		{
			case 16 :
				//xres_bits = var->xres_virtual * (16 >> 3);
				xres_bits = var->xres_virtual << 1;
				break;

			case 24 :
			case 32 :
				//xres_bits = var->xres_virtual * (32 >> 3);
				xres_bits = var->xres_virtual << 2;
				break;
				
			default:
				xres_bits = var->xres_virtual * (var->bits_per_pixel >> 3);	
				break;
		}
		start_addr = fix->smem_start + (xres_bits * var->yoffset);
		end_addr   = start_addr      + (xres_bits * var->yres);
	}
