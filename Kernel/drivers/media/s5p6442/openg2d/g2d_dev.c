...
#define G2D_DEBUG 1
#ifdef G2D_DEBUG
#define DBG(format, args...) printk("%s: " format, DRIVER_NAME, ## args)
#else
#define DBG(format, args...)
#endif
#define ERR(format, args...) printk("%s: " format, DRIVER_NAME, ## args)
#define WARNING(format, args...) printk("%s: " format, DRIVER_NAME, ## args)
#define INFO(format, args...) printk("%s: " format, DRIVER_NAME, ## args)
...
static int s3c_g2d_probe(struct platform_device *pdev)
{
	...
	/* get the clock */
	data->clock = clk_get(&pdev->dev, "clk_g2d");
	if (data->clock == NULL) {
